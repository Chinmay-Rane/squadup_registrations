...
// Use the DIRECT_URL you provided earlier
...
async function runMigration() {
  try {
    ...
    const sql = `
      ...
      DROP POLICY IF EXISTS "Enable read access for all users" ON public.form_config;
      CREATE POLICY "Enable read access for all users" ON public.form_config
          AS PERMISSIVE FOR SELECT
          TO public
          USING (true);

      DROP POLICY IF EXISTS "Enable update for all users" ON public.form_config;
